...
mysql -uroot -p"${WP_ROOT_PASSWORD}" <<EOF
	...
	CREATE USER IF NOT EXISTS '$DB_ADMIN_USER'@'localhost' IDENTIFIED BY '$WP_ADMIN_PASSWORD';
	...
	CREATE USER IF NOT EXISTS '$DB_DEFAULT_USER'@'localhost' IDENTIFIED BY '$DB_DEFAULT_USER_PASSWORD';
	GRANT SELECT ON $DB_NAME.* TO '$DB_DEFAULT_USER'@'localhost';
